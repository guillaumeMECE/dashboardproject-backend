...
const process = async (params) => {
    try {
        const data = await UserModel.findById(params.id).exec();
        const resSensor = await SensorModel.find({ 'userID': data._id })
        return resSensor;
    } catch (error) {
 ...
 * LOGIC :
 */
const userReadSensor = async (req, res) => {
    try {
